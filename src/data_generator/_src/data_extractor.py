...
class DataExtractor:
    ...
    def load_csv_files(self, path: str = None, display=False) -> list:
        if path is None:
            path = self.extract_from
        csv_files = glob(path + f"{self.sample_type}*.csv")

        if display:
            print(f"Loading {len(csv_files)} csv files to extract fingerprint from")
    ...
    def convert_spots_to_fingerprints(self, rgbs_by_type:list[dict[list[list[float]]]])->dict[list[list[float]]]:
        ...
        for type_ in appended_spots:

            # separate the corr RGB values for each block type
            spot1_corr_rgbs, spot2_corr_rgbs = self.separate_spots(appended_spots[type_])
            ...
            fingerprint_by_block_type[type_] = block_type_fingerprint

        return fingerprint_by_block_type
    
    def separate_spots(self, blocks):
        spot1_corr_rgbs = []
        spot2_corr_rgbs = []
        for block in blocks:
    ...
    def extract(self, path: str = None, display:bool=False) -> dict:
        ...
        if display:
            display_fingerprint(fingerprints)
        
        return fingerprints
    
    def append_spots(self, rgbs_by_type):
        # initializing map with empty lists
        appended_spots = {type_: [] for image in rgbs_by_type for type_ in image}
    ...
    def extract_spots_from_multiple_csv(self, csv_files:list[str]):
        spots=[]
    ...
    def extract_spots_from_single_csv(self, path: str) -> dict[list[list[float]]]:
        """
    ...
    def extract_corr_rgbs(self, data: list) -> dict: 
        """ extract the corr RGB values from the csv file of each blocktype"""
        rgbs_by_type = {}
        for line in data[1:]:
            # gets one row from csv
            row = csv.reader(StringIO(line), delimiter=',', quotechar='"').__next__()
            ...
            rgbs_by_type[block_type].append([format_line(row, 0), format_line(row, 1)])
